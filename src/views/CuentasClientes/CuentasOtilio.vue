      ...
      async actualizarCuentasPosteriores(fechaActual) {
        try {
          ...
          for (let i = 0; i < cuentasOrdenadas.length; i++) {
            ...
            const cuentaRef = doc(db, 'cuentasOtilio', cuenta.id);
      ...
      async guardarNota() {
        try {
          ...
          if (id && isEditing) {
            ...
            docRef = doc(db, 'cuentasOtilio', id);
        ...
        try {
          ...
          if (abono.abonoGeneralId) {
            try {
              ...
              const abonoGeneralRef = doc(db, 'abonosGeneralesOtilio', abono.abonoGeneralId);
                ...
                const actualizacionesCuentas = querySnapshot.docs.map(async (docSnapshot) => {
                  ...
                  if (cuentaData.abonos) {
                    ...
                    if (abonosActualizados.length !== cuentaData.abonos.length) {
                      await updateDoc(doc(db, 'cuentasOtilio', docSnapshot.id), {
          ...
          for (let i = 0; i < cuentasOrdenadas.length; i++) {
            ...
            actualizaciones.push(updateDoc(doc(db, 'cuentasOtilio', cuenta.id), {
